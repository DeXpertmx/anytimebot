...
export default function OnboardingPage() {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      toast({ title: 'Profile configured', description: 'Your public booking URL is ready.' });
  ...
  return <div className="mx-auto max-w-2xl space-y-6">
    <div><h1 className="text-3xl font-bold">Set up your booking profile</h1><p className="mt-2 text-gray-600">Choose your public username, timezone, and weekly availability.</p></div>
    <form onSubmit={submit} className="space-y-6 rounded-lg border bg-white p-6 shadow-sm">
      <div><Label htmlFor="onboarding-username">Public username</Label><Input id="onboarding-username" value={username} onChange={e => setUsername(e.target.value.toLowerCase())} placeholder="juanperez" required minLength={3} maxLength={20} /><p className="mt-1 text-sm text-gray-500">Your public URL will start with anytimebot.app/{username || 'username'}/</p></div>
      <div><Label>Timezone</Label><TimezoneSelect value={timezone} onValueChange={setTimezone} /></div>
      <div className="rounded border p-4"><p className="font-medium">Google Calendar</p><p className="text-sm text-gray-600">Connect it so availability and meetings stay synchronized.</p><Button type="button" variant="outline" className="mt-3" onClick={connectCalendar}>{calendarConnected ? 'Connected' : 'Connect Google Calendar'}</Button></div>
      <div className="space-y-3"><Label>Weekly availability</Label>{availability.map((slot, index) => <div key={slot.dayOfWeek} className="grid grid-cols-[1fr_auto_auto] items-center gap-3"><label className="flex items-center gap-2"><input type="checkbox" checked={slot.isAvailable} onChange={e => setAvailability(current => current.map((item, i) => i === index ? { ...item, isAvailable: e.target.checked } : item))} />{days[index]}</label><Input type="time" value={slot.startTime} disabled={!slot.isAvailable} onChange={e => setAvailability(current => current.map((item, i) => i === index ? { ...item, startTime: e.target.value } : item))} /><Input type="time" value={slot.endTime} disabled={!slot.isAvailable} onChange={e => setAvailability(current => current.map((item, i) => i === index ? { ...item, endTime: e.target.value } : item))} /></div>)}</div>
      <Button type="submit" disabled={saving}>{saving ? 'Saving...' : 'Finish setup'}</Button>
    </form>
  </div>;
}
