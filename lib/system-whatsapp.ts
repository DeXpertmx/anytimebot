...
export async function activateSystemWhatsApp(
  ...
  // answers 403 "name already in use" when creating a duplicate, and the
  // stored config may have been lost (e.g. after a DB restore).
  const existing = await fetchImpl(`${baseUrl}/instance/connectionState/${instanceName}`, {
    method: 'GET',
    headers: { apikey: apiKey },
  });
  if (!existing.ok && existing.status !== 404) {
    throw new Error(`Failed to check system WhatsApp instance (${existing.status})`);
  }
