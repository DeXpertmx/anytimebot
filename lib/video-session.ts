...
interface CreateVideoSessionParams {
  ...
  /** Precomputed meeting URLs (Zoom/Teams/Custom) generated before the session row. */
  meetingUrls?: {
    roomUrl?: string;
    hostRoomUrl?: string;
...
export async function createVideoSession(params: CreateVideoSessionParams) {
  const { bookingId, provider, eventTypeConfig, meetingDetails, meetingUrls } = params;
  ...
  let roomUrl = meetingUrls?.roomUrl || '';
  let hostRoomUrl = meetingUrls?.hostRoomUrl || '';
  let dailyRoomId = null;
  let roomName = meetingUrls?.roomName || null;
  ...
  if (provider === VideoProvider.DAILY) {
    ...
    // The actual meeting URL is generated by Google Calendar during booking.
    // This provider is not embeddable; use the stored calendar link when available.
    roomUrl = 'https://meet.google.com/';
  } else if (
    provider === VideoProvider.ZOOM ||
    provider === VideoProvider.TEAMS ||
    provider === VideoProvider.CUSTOM
  ) {
    // The meeting URL is generated before this call (Zoom/Teams via the
    // tenant's connection, CUSTOM from the event type). If nothing was
    // resolved, the session row is still created with the stored link so the
    // meeting room can show the correct state.
    if (!roomUrl) {
      roomUrl = meetingUrls?.roomUrl || '';
    }
  }
